-- ============================================================
-- Map Performance Optimization: 3 Quick Wins
-- 1. Add missing columns to map_ready_tables (eliminate 2nd query)
-- 2. Add partial spatial index for story viewport queries
-- ============================================================

-- ===== FIX 1: Update map_ready_tables view =====
-- Add marker_image_url, marker_emoji, image_url, images columns
-- so the client doesn't need a second enrichment query
...
SELECT 
    ...
    -- ✅ Previously missing columns (required separate enrichment query)
    t.marker_image_url,
    t.marker_emoji,
    t.image_url,
    t.images,
...
-- ===== FIX 2: Add partial spatial index for story queries =====
-- The map_live_stories_view queries posts WHERE is_story = true
-- with bounding box on latitude/longitude. This index makes it instant.

CREATE INDEX IF NOT EXISTS idx_posts_story_viewport 
ON public.posts(latitude, longitude) 
WHERE is_story = true;

-- Also add a compound index for the 24h window (if the view filters by time)
CREATE INDEX IF NOT EXISTS idx_posts_story_recent
ON public.posts(created_at DESC)
WHERE is_story = true AND latitude IS NOT NULL;
