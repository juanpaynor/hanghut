-- Drop the function first because we are changing the return type (adding event_id)
DROP FUNCTION IF EXISTS get_philippines_feed(integer, timestamp with time zone, uuid, double precision, double precision);
-- Also drop the without timezone version just in case, as hinted by the error
DROP FUNCTION IF EXISTS get_philippines_feed(integer, timestamp without time zone, uuid, double precision, double precision);

CREATE OR REPLACE FUNCTION get_philippines_feed(
    p_limit INT DEFAULT 20,
    p_cursor TIMESTAMP DEFAULT NULL, -- Note: keeping consistent with existing param types
