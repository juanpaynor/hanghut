-- Add marker_model column to tables table
ALTER TABLE public.tables ADD COLUMN IF NOT EXISTS marker_model TEXT;

-- Create index for faster filtering/retrieval (Optimized)
CREATE INDEX IF NOT EXISTS idx_tables_marker_model ON public.tables(marker_model);

-- Update map_ready_tables view to include the new column
...
WHERE t.status = 'open'
  AND t.datetime > NOW()
GROUP BY t.id, u.display_name, up.photo_url, u.trust_score, t.marker_model;
