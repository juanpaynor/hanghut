-- Create RPC for fetching the main feed efficiently
-- Replaces complex client-side query construction
...
CREATE OR REPLACE FUNCTION get_main_feed(
    ...
    p_h3_cells TEXT[] DEFAULT NULL -- Pass calculated cells from client to save DB CPU, or allow null
)
RETURNS TABLE (
    id UUID,
    content TEXT,
    ...
    -- User Info (Json object to match Dart expectation or flattened)
    user_data JSONB,
...
BEGIN
    -- 1. Cache following IDs to avoid correlated subqueries (O(1) vs O(N))
    ...
    SELECT 
        ...
        p.content,
        p.image_url,
        p.image_urls::TEXT[], -- Cast array to text[] just in case
        p.created_at,
        p.user_id,
        p.post_type,
        p.metadata,
        ...
        -- Aggregates (Correlated subqueries are fast enough for paginated feeds, 
        -- faster than JOIN GROUP BY on whole table with proper indexes)
    ...
    WHERE 
        -- 1. Location Filter (if cells provided and lat/lng provided)
        (
            p_h3_cells IS NULL 
            OR 
        ...
        -- 2. Visibility Filter
        (
            -- Everyone sees their own posts
            (v_current_user_id IS NOT NULL AND p.user_id = v_current_user_id)
            OR
            -- Public posts
            (p.visibility = 'public')
            OR
            -- Follower only posts (Optimized check)
            (
